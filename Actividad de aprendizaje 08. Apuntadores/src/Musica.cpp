...
std::string AllSong; AllSong += SongName; AllSong += " | ";
AllSong += SongAutor; AllSong += " | ";
AllSong += SongRanking;
return AllSong;
}
...
std::istream& operator >> (std::istream& is, Songs& s){ is >> s.SongAutor;
is >> s.SongAutor; is >> s.SongRanking; return is;
}
